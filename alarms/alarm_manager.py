...
class Alarm_Manager(Thread):
        ...
        with open(get_path(config['CONFIG_FILE'])) as file:
            ...
            for alarm in alarm_settings:
                if alarm['active'] == "True":
                    ...
                    log.info("Alarm not activated: " +
                             alarm['type'] + " because value not set to \"True\"")
            ...
            try:
                ...
                for i in range(5000):  # Take a break and clean house every 5000 requests handled
                    ...
                    if data['type'] == 'pokemon':
                        log.debug("Request processing for Pokemon #%s" %
                                  data['message']['pokemon_id'])
                        self.trigger_pokemon(data['message'])
                        log.debug("Finished processing for Pokemon #%s" %
                                  data['message']['pokemon_id'])
                    elif data['type'] == 'pokestop':
                        log.debug("Request processing for Pokestop %s" %
                                  data['message']['pokestop_id'])
                        self.trigger_pokestop(data['message'])
                        log.debug("Finished processing for Pokestop %s" %
                                  data['message']['pokestop_id'])
                    elif data['type'] == 'gym' or data['type'] == 'gym_details':
                        log.debug("Request processing for Gym %s" % data[
                                  'message'].get('gym_id', data['message'].get('id')))
                        self.trigger_gym(data['message'])
                        log.debug("Finished processing for Gym %s" % data[
                                  'message'].get('gym_id', data['message'].get('id')))
                    else:
    ...
    def trigger_pokemon(self, pkmn):
        ...
        self.pokemon["{}{}".format(pkmn['encounter_id'], pkmn[
                                   'may_extend'])] = dissapear_time
        ...
        if seconds_left < config['TIME_LIMIT']:
            ...
            log.debug("Time left must be %f, but was %f." %
                      (config['TIME_LIMIT'], seconds_left))
            return
        ...
        if dist >= config["NOTIFY_LIST"][pkmn_id]:
            ...
            log.debug("Pokemon must be less than %d, but was %d." %
                      (config["NOTIFY_LIST"][pkmn_id], dist))
            return
        ...
        guess = ""
        try:
            payload = {'q': pkmn['spawnpoint_id']}
            r = requests.get(config['SP_URL'], params=payload)
            result = json.loads(r.text)
            result = result[0]
            if result['c_spawn'] is not None:
                s = time.strptime(result['c_spawn'], "%Y-%m-%d %H:%M:%S")
                guess = guess + time.strftime('%H:%M:%S', s)
                if result['c_hide'] is not None:
                    h = time.strptime(result['c_hide'], "%Y-%m-%d %H:%M:%S")
                    r = time.strptime(result['c_reappear'], "%Y-%m-%d %H:%M:%S")
                    guess = guess + "-" + time.strftime('%H:%M:%S', h)
                    guess = guess + "/" + time.strftime('%H:%M:%S', r)
                d = time.strptime(result['c_despawn'], "%Y-%m-%d %H:%M:%S")
                guess = guess + "-" + time.strftime('%H:%M:%S', d)
            if result['l_type'] is not None:
                t = result['l_type']
                guess = guess + " ({})".format(t)
        except Exception as e:
            log.error("Read SP: {}".format(e))
        print(guess)
        ...
        try:
            pkmn_info['iv_a'] = str(
                pkmn.get('attack',  pkmn.get('individual_attack', 0)))
            if pkmn_info['iv_a'] == 'None':
                pkmn_info['iv_a'] = "0"
        except:
            pkmn_info['iv_a'] = "0"
        try:
            pkmn_info['iv_s'] = str(
                pkmn.get('stamina', pkmn.get('individual_stamina', 0)))
            if pkmn_info['iv_s'] == 'None':
                pkmn_info['iv_s'] = "0"
        except:
            pkmn_info['iv_s'] = "0"
        try:
            pkmn_info['iv_d'] = str(
                pkmn.get('defense', pkmn.get('individual_defense', 0)))
            if pkmn_info['iv_d'] == 'None':
                pkmn_info['iv_d'] = "0"
        except:
            pkmn_info['iv_d'] = "0"
        try:
            pkmn_info['iv'] = "%0.2f" % ((float(pkmn_info[
                                         'iv_a']) + float(pkmn_info['iv_s']) + float(pkmn_info['iv_d'])) / 45.0 * 100)
        except:
            pkmn_info['iv'] = "??"
            pass
        try:
            pkmn_info['move_1'] = get_move_name(
                pkmn.get('move_1', pkmn.get('move_1', 0)))
        except:
            pass
        try:
            pkmn_info['move_2'] = get_move_name(
                pkmn.get('move_2', pkmn.get('move_2', 0)))
        except:
            pass
    ...
    def trigger_pokestop(self, stop):
        ...
        if dist >= self.stop_list['lured']:
            ...
            log.debug("Pokestop must be less than %d, but was %d." %
                      (self.stop_list['lured'], dist))
            return
    ...
    def trigger_gym(self, gym):
        ...
        max_dist = max(self.gym_list.get("From_%s" % old_team, -1),
                       self.gym_list.get("To_%s" % new_team, -1))
        if max_dist is -1:
        ...
        if dist >= max_dist:
            ...
            log.debug("Gym must be less than %d, but was %d." %
                      (max_dist, dist))
            return
